...
export async function POST(request: Request) {
  ...
  if (!courseMaterial || !jobStandard) {
    return apiError("MISSING_INPUT", "请先填写课程资料和岗位标准。", 400);
  }
  ...
  if (!apiKey) {
    return apiError(
      ...
      "服务端未配置 DEEPSEEK_API_KEY，暂时无法生成分层任务。",
      503,
    );
  }
        ...
        messages: [
          {
            role: "system",
            content:
              "你是职业教育实训设计专家。只输出 JSON，不要输出 Markdown。JSON 必须是 {\"tasks\":[...]}。tasks 必须依次包含基础、进阶、挑战三个任务。每个任务必须包含 title、objective、description、submissionRequirements、evaluationFocus、rubric。rubric 必须且只能包含五个维度：岗位需求分析、AI 应用设计、模型接口调用、效果评估与调优、安全合规与文档表达。五个维度分值固定为 20、25、25、20、10。每个 rubric item 必须包含 dimension、criteria、excellentStandard、passStandard、deductionPoint。",
          },
          {
            role: "user",
            content: `课程资料：\n${courseMaterial}\n\n岗位标准：\n${jobStandard}\n\n请生成三层实训任务和可编辑评分量规。`,
  ...
  if (!upstreamResponse.ok) {
    return apiError(
      "UPSTREAM_FAILED",
      `AI 生成服务调用失败（${upstreamResponse.status}）。`,
      502,
    );
  }
  ...
  if (!content) {
    return apiError("EMPTY_MODEL_RESPONSE", "AI 生成结果为空，请稍后重试。", 502);
  }
